...
const DriverDashboard = () => {
  ...
  const toggleAvailability = async () => {
    try {
      const newAvailability = !dashboardData.profile.is_available_for_hire;
      
      const { error } = await supabase
        .from('driver_profiles')
        .update({ 
          is_available_for_hire: newAvailability,
  ...
  const statsCards = [
    {
      ...
      value: dashboardData?.stats?.rating ? 
        `${dashboardData.stats.rating.toFixed(1)}/5` : 'No ratings',
      icon: StarIcon,
      color: 'yellow',
      link: '/driver/reviews'
    },
    {
      title: 'Total Trips',
      value: dashboardData?.stats?.total_trips || 0,
      icon: CarIcon,
      color: 'blue',
      link: '/driver/rentals'
    },
    {
      ...
      value: dashboardData?.stats?.active_assignments || 0,
      icon: UserIcon,
      color: 'green',
    ...
    },
    {
      ...
      value: dashboardData?.stats?.total_earnings ? 
        `₹${dashboardData.stats.total_earnings.toLocaleString()}` : '₹0',
  ...
  const quickActions = [
    ...
    {
      title: 'View Rentals',
      description: 'Manage your rental trips',
      icon: ClockIcon,
      color: 'purple',
      link: '/driver/rentals'
    },
          ...
          <div className="mt-4 flex md:mt-0 md:ml-4">
            <button
              ...
              className={`inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                dashboardData?.profile?.is_available_for_hire
                  ? 'bg-green-600 hover:bg-green-700 focus:ring-green-500'
                  : 'bg-red-600 hover:bg-red-700 focus:ring-red-500'
              }`}
            >
              {dashboardData?.profile?.is_available_for_hire ? 'Available' : 'Unavailable'}
            </button>
          </div>
        </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-yellow-700">
                  <p>
                    Complete your driver profile to start receiving job offers from businesses.
                  </p>
                </div>
                <div className="mt-4">
